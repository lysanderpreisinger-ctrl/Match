...
// ===== Stripe Price IDs (bitte ersetzen, wo nötig) =====
const PRICE_STANDARD_MONTHLY_89 = 'price_1RufD3Bh0hSWnazMm76eBxCA';
const PRICE_PREMIUM_MONTHLY_199 = 'price_1RufFVBh0hSWnazMyCvGGgv2';
const PRICE_CREDITS_3   = 'price_XXX_CREDITS_3';
const PRICE_CREDITS_10  = 'price_XXX_CREDITS_10';
const PRICE_CREDITS_25  = 'price_XXX_CREDITS_25';
...
export default function SubscriptionScreen() {
  ...
  // aktueller Kaufkontext
  const currentPurchaseRef = useRef(null);

  // ✅ FIX: kein TS-Generic in .js
  const [activeTab, setActiveTab] = useState('subs'); // 'subs' | 'credits'
  ...
  const startCheckout = useCallback(async ({ priceId, mode }) => {
    ...
    if (!res.ok) throw new Error(data?.error || t('billing.errors.createSession'));
    if (!data?.url) throw new Error(t('billing.errors.noCheckoutUrl'));
    return data.url;
  }, []);
